namespace ECSToolbox
{
	...
	public static class EntityCollisionUtility
	{
		...
		public static PhysicsCollider Convert(TerrainCollider terrain, CollisionFilter filter)
		{
			...
			using NativeList<CompoundCollider.ColliderBlobInstance> colliders = new(2, Allocator.Temp);
		...
		{
			NativeList<CompoundCollider.ColliderBlobInstance> treeColliders =
				new(terrainData.treeInstanceCount, Allocator.Temp);
			...
			for (int i = 0; i < terrainData.treeInstanceCount; i++)
			{
				...
				BlobAssetReference<Unity.Physics.Collider> collider = CreateCollider(unityCollider, filter);

				float4x4 trs = float4x4.TRS(Vector3.Scale(tree.position, terrainData.size),
					quaternion.RotateY(tree.rotation), new float3(tree.widthScale, tree.heightScale, tree.widthScale));
			...
			BlobAssetReference<Unity.Physics.Collider> treeCompound = CompoundCollider.Create(treeColliders.AsArray());
		...
		static BlobAssetReference<Unity.Physics.Collider> CreateCollider(Collider unityCollider, CollisionFilter filter)
		{
			...
			if (unityCollider is MeshCollider mc)
			{
				if (mc.convex)
				{
					...
					BlobAssetReference<Unity.Physics.Collider> ccc = ConvexCollider.Create(verts.Reinterpret<float3>(),
		...
		static Collider GetColliderFromPrefab(GameObject protoPrefab) => protoPrefab.GetComponentInChildren<Collider>();
			...
			CollisionFilter filter)
		{
			...
			NativeArray<float> colliderHeights = new(terrainData.heightmapResolution * terrainData.heightmapResolution,
				Allocator.TempJob);

			NativeArray<bool> colliderHoles = new(
				terrainData.holesResolution * terrainData.holesResolution, Allocator.TempJob);
			...
			BlobAssetReference<Unity.Physics.Collider> c = Unity.Physics.TerrainCollider.Create(colliderHeights,
				colliderHoles, size, scale, Unity.Physics.TerrainCollider.CollisionMethod.Triangles, filter);
